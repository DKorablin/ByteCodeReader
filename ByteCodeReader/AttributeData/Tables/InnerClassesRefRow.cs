...
namespace AlphaOmega.Debug.AttributeData
{
	...
	public class InnerClassesRefRow : BaseRow<String>
	{
		...
		/// The constant_pool entry at that index must be a <see cref="Jvm.CONSTANT_Class_info"/> structure (§4.4.1) representing C.
		...
		/// Otherwise, the value of the outer_class_info_index item must be a valid index into the constant_pool table, and the entry at that index must be a <see cref="Jvm.CONSTANT_Class_info"/> (§4.4.1) structure representing the class or interface of which C is a member.
		/// </summary>
		...
		/// Otherwise, the value of the inner_name_index item must be a valid index into the constant_pool table, and the entry at that index must be a <see cref="Jvm.CONSTANT_Utf8_info"/> (§4.4.7) structure that represents the original simple name of C, as given in the source code from which this class file was compiled.
		/// </summary>
